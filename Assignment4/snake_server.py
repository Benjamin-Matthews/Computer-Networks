...
# dictionary to score currently connected clients
clients = {}

#fuction to proccess client data 
def process_client_data(data, unique_id):
...
# function to bhandle client threads
def client_thread(conn, unique_id):
    ...
    while True:
        try:
            ...
            if not data:
                print(f"Client {unique_id} disconnected")
                game.remove_player(unique_id)
                conn.close()
                clients.pop(unique_id)
                break

            process_client_data(data, unique_id)

            conn.send(game_state.encode())

        except Exception as e:
            print(f"Error processing client {unique_id}: {str(e)}")
            break
